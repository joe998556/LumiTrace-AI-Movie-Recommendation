...
function getLlmConfig() {
  return window.LumiTraceRecs?.getLlmConfig() || null;
}
...
function updateConnections() {
  ...
  const customAllowed = !window.__lumitraceRemoteLocked && Boolean(getSearchUrlOverride());
  set("connBert", backendHasBert || customAllowed, customAllowed ? "custom URL" : backendHasBert ? "server locked" : "off");
...
async function runZeroShotPlaylist() {
  ...
  const ratings = getRatings();
  const payload = window.LumiTraceRecs.buildPayload({
    favorites: favs,
    ratings,
    prompt,
    topK: 30,
    diversity: window.__lumiDiversity ?? 0.55,
    language,
    genre,
  });
...
function renderRecGrid(movies) {
  ...
  document.dispatchEvent(new CustomEvent("lumitrace:recommendations", { detail: { movies } }));
}
...
function makeRecCard(movie) {
  ...
  card.className = "poster-card";
  card.dataset.movieId = String(movie.id);
  if (movie.evidence) card.dataset.evidence = JSON.stringify(movie.evidence);
  const active = favSet.has(movie.id);
  const year = movie.release_date ? movie.release_date.slice(0, 4) : "";
  const s = movie.vote_average;
  card.innerHTML = `
    ...
    <div class="poster-content">
      ...
      <div class="taste-feedback-row" aria-label="Recommendation feedback">
        <button class="taste-feedback-btn" data-direction="more" type="button">More like this</button>
        <button class="taste-feedback-btn" data-direction="less" type="button">Less like this</button>
  ...
  card.querySelectorAll(".taste-feedback-btn").forEach((button) => button.addEventListener("click", (event) => {
    event.stopPropagation();
    document.dispatchEvent(new CustomEvent("lumitrace:feedback", { detail: { movie, direction: button.dataset.direction } }));
  }));
  card.querySelector(".compare-btn").addEventListener("click", (event) => {
    event.stopPropagation();
    document.dispatchEvent(new CustomEvent("lumitrace:compare", { detail: { movie } }));
  });
  return card;
}
...
function showExplanation(movie) {
  ...
  const evidenceReasons = (window.LumiTraceRecs?.evidenceRows(movie, GENRE_NAMES) || []).map((row) => ({ icon: "AI", title: row.label, detail: row.value }));
  const allReasons = evidenceReasons.length ? evidenceReasons.slice(0, 4) : reasons;
  document.getElementById("explanationBody").innerHTML = `
    <div class="explanation-preview">
      ...
      <div>
        ...
        <p class="explanation-overview line-clamp-3">Evidence only. This explanation intentionally avoids plot details.</p>
      </div>
    </div>
    <div class="reason-list">
      ${allReasons.length ? allReasons.map((reason) => `
